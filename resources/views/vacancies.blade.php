                ...
                @foreach($vacancies as $vacancy)
                    <div class="vacancy-item {{ $theme === 'dark' ? 'bg-gray-300' : 'bg-gray-100' }} rounded-lg shadow-sm flex flex-col border border-gray-200 overflow-hidden"
                        ...
                        <div class="p-6 flex flex-col">
                            ...
                            <a href="{{ route(app()->getLocale() . '.vacancy.show', $vacancy->slug) }}" class="text-custom-green hover:text-custom-lightergreen font-medium text-sm self-end">{{ __('general.see_more') }} →</a>
                        </div>
                    </div>
                @endforeach
            @else

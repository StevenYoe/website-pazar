...
<body data-page="{{ Route::currentRouteName() }}" data-theme="{{ $theme }}">
    <script>
    ...
    <header class="landing relative">
        <!-- Navbar Section -->
        <nav id="navbar" class="fixed w-full z-20 top-0 start-0 {{ $theme === 'dark' ? 'border-gray-600 text-white' : 'border-gray-200' }} @if(Route::currentRouteName() == 'products.show') bg-custom-red @endif" data-breakpoint="793">
            <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
                <a href="{{ app()->getLocale() == 'en' ? '/en' : '/id' }}" class="flex items-center space-x-3 rtl:space-x-reverse">
                    <img src="{{ asset('img/web/Logo.png') }}" class="h-14" alt="Pazar Logo">
                    <span class="self-center text-2xl font-semibold whitespace-nowrap {{ $theme === 'dark' ? 'text-white' : 'text-white' }}"></span>
                </a>

                <!-- Mobile menu button -->
                <button data-collapse-toggle="navbar-sticky" type="button" class="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg md1:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 {{ $theme === 'dark' ? 'text-gray-400 hover:bg-gray-700 focus:ring-gray-600' : 'text-gray-500 hover:bg-gray-100 focus:ring-gray-200' }}" aria-controls="navbar-sticky" aria-expanded="false">
                                    ...
                                    <ul class="py-2 font-medium" role="none">
                                        @foreach(config('app.available_locales') as $locale => $language)
                                            <li>
                                                ...
                                                class="block px-4 py-2 text-sm {{ $theme === 'dark' ? 'text-white' : 'text-gray-700' }} 
                                                hover:bg-custom-lightergreen {{ $theme === 'dark' ? 'hover:bg-custom-lightergreen hover:text-white' : 'hover:bg-custom-lightergreen hover:text-white' }} 
                                                {{ app()->getLocale() == $locale ? ($theme === 'dark' ? 'bg-gray-600' : 'bg-gray-100') : '' }}" 
                                                role="menuitem">
                                                    <div class="inline-flex items-center">
                                                        <img src="{{ asset($language['flag']) }}" alt="{{ $language['name'] }} Flag" 
                                                            class="h-3.5 w-3.5 rounded-full me-2"> {{ $language['native'] }}
                                                    </div>
                                                </a>
                                            </li>
                                        @endforeach
                ...
                <div class="flex flex-col items-center md:items-start">
                    ...
                    @if(isset($footerData) && isset($footerData['address']) && $footerData['address'])
                        ...
                        <a href="{{ $footerData['address']->f_link ?? '#' }}" target="_blank" class="text-center md:text-left {{ $theme === 'dark' ? 'text-gray-200' : 'text-white' }} hover:text-custom-lightergreen">
                            <p class="font-medium">{{ $footerData['address']->current_label }}</p>
                            <p class="mt-1 whitespace-normal break-words">
                                {!! nl2br(e($footerData['address']->current_description ?? '')) !!}
                            </p>
                        </a>
                    @endif
                </div>
                        ...
                        @foreach($footerData['contacts'] as $contact)
                            <div class="flex items-center mb-3">
                                @if(isset($contact->f_icon) && $contact->f_icon)
                                    <img src="{{ $contact->f_icon }}" alt="{{ $contact->current_label ?? 'Contact' }} Icon" class="h-5 w-5 mr-2">
                                @endif
                                <a href="{{ $contact->f_link ?? '#' }}" class="{{ $theme === 'dark' ? 'text-gray-200' : 'text-white' }} hover:text-custom-lightergreen">
                                    <span>{{ $contact->current_description ?? $contact->current_label ?? 'Contact Us' }}</span>
                                </a>
                            </div>
                        @endforeach
                    @endif
